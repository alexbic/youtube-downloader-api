...
- Приоритет: `webhook.headers` из запроса > глобальные `WEBHOOK_HEADERS`
...
| `DEFAULT_WEBHOOK_URL` | — | Резервный URL webhook (async). Используется когда запрос не содержит объект `webhook`. Должен начинаться с http(s)://, < 2048 символов. |
...
**Конфигурация:**
- Используйте `DEFAULT_WEBHOOK_URL` для установки fallback webhook для всех async задач без явного объекта `webhook`
