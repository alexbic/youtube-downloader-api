...
- 🔗 **Поддержка webhook** - POST уведомления при завершении задачи
...
- 📦 **Поддержка Redis** - хранилище задач для нескольких воркеров
- 🔒 **Поддержка cookies** - обход ограничений YouTube
- 🧹 **Автоочистка** - настраиваемый TTL для файлов задач
...
| `WEBHOOK_RETRY_ATTEMPTS` | `3` | Максимум попыток доставки webhook. |
| `WEBHOOK_RETRY_INTERVAL_SECONDS` | `5` | Задержка между повторами webhook (секунды). |
| `WEBHOOK_TIMEOUT_SECONDS` | `8` | Таймаут запроса webhook (секунды). |
| `DEFAULT_WEBHOOK_URL` | — | Резервный URL webhook (async). |
| `WEBHOOK_HEADERS` | — | Дополнительные заголовки для webhook POST. |
| **Логирование** |||
